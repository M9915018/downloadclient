...
//            LogUtils.info("{} 開始下载檔案{}.", Thread.currentThread().getName(),httpFileName);
...
//            }
//            LogUtils.info("{} 已经下载完毕.", Thread.currentThread().getName());
...
try (ReadableByteChannel rbc = Channels.newChannel(httpUrlConnection.getInputStream());
     FileChannel fileChannel = new RandomAccessFile(httpFileName, "rw").getChannel()) {

    LogUtils.info("{} 開始下载檔案{}.", Thread.currentThread().getName(),httpFileName);
    fileChannel.position(localFileContentLength);
    ...
    while ((bytesRead = rbc.read(buffer)) != -1) {
        ...
        LogThread.DOWNLOAD_SIZE.addAndGet(bytesRead);
        buffer.clear();
    }
    LogUtils.info("{} 已经下载完毕.", Thread.currentThread().getName());
} catch (FileNotFoundException e) {
